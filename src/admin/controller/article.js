...
export default class extends Base {
  ...
  //翻译接口
  async transAction() {
    var title = this.post("t");
    console.log(title);

    var routeTransAPI = 'http://api.fanyi.baidu.com/api/trans/vip/translate';
    var appid = '20160218000012560';
    var key = 'NblJ36jLqoL8mKEsevzh';
  ...
  uploadFile(uptoken, key, localFile) {
    ...
    qiniu.io.putFile(uptoken, key, localFile, extra, function (err, ret) {
      if (!err) {
        rp({
          uri: "http://7xs2uw.com1.z0.glb.clouddn.com/" + ret.key + "?imageInfo",
          json: true
        }).then(function (repos) {
          // 上传成功， 处理返回值
          ...
          let backjson = {
            ...
            url: "http://7xs2uw.com1.z0.glb.clouddn.com/" + ret.key + " =" + repos.width + "x" + repos.height       // 上传成功时才返回
          };
          self.end(backjson);
        })
      } else {
